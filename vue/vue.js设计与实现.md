...
对于一个对象来说，只读意味着既不可以设置对象的属性值，也不可以删除对象的属性。这样就实现了对数据的保护，例如**组件接收到的 props 对象应该是一个只读数据。**
...
当组件自身状态发生变化时，我们需要组件能够自更新。为此我们需要将整个渲染任务（render、patch等）包装到一个effect中。这样，一旦组件自身的响应式数据发生变化，组件就会自动重新执行渲染函数，从而完成更新。
